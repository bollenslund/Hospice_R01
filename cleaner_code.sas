...
proc sort data=hospice_base4 out=hospice_base_rev;
	by clm_id;
run;
...
proc sql;
	create table hospice_base_rev_1
	  as select *
	    from hospice_base_rev a
		  left join hospice_revenue3 b
		  	on a.clm_id = b.clm_id;
quit;

proc freq data=hospice_base_rev_1;
	table rev_code;
run;

data hospice_base_rev_2;
	set hospice_base_rev_1;
		rev_days = (CLM_THRU_DT - CLM_FROM_DT)+1;
run;

proc sort data=hospice_base_rev_2 out=hospice_base5;

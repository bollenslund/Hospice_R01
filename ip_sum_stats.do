...
icu_stay_ind icu_stay_cnt icu_stay_days hosp_death ip_tot_cost
 ...
 frmttable using "J:\Geriatrics\Geri\Hospice Project\output\ip_sum_stats", ///
	...
	"ICU Stay - count" \ "ICU Stay - days" \ "Hospital death" \ "Total cost IP claims") sdec(3) replace
...
icu_stay_ind icu_stay_cnt icu_stay_days hosp_death ip_tot_cost cost_per_adm
 ...
 mat list mean_vars
 frmttable using "J:\Geriatrics\Geri\Hospice Project\output\ip_sum_stats", ///
	statmat(mean_vars) title("Means of hospital use variables from IP claims - for those with at least 1 hospital admission") ///
	ctitle("", "Mean", "Median") ///
	rtitle( "Hosp adm - count" \ "Hosp adm - days" \ "ED visit?" ///
	\ "ED visit count" \ "ICU Stay?" \ "ICU Stay - count" \ "ICU Stay - days" \ ///
	"Hospital death"\ "Total cost IP claims"\ "Mean cost per admission") sdec(3) addtable	
